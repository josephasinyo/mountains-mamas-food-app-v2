// Run this script to add the mailing_address column to tour_companies in Supabase
// Usage: SUPABASE_ACCESS_TOKEN=your_token node scripts/apply_tour_companies_mailing_address_migration.js
...
(async () => {
    if (!TOKEN) {
        ...
        console.log('ALTER TABLE tour_companies ADD COLUMN IF NOT EXISTS mailing_address TEXT;');
        process.exit(1);
    }
    console.log('Running tour_companies mailing_address migration...\n');
    for (const q of queries) {
        await runQuery(q);
    }
    console.log('\nDone! The mailing_address column has been added to tour_companies.');
})();
