...
import { 
    ...
    Cookie, Utensils, FileText, ArrowUp, ArrowDown, GripVertical
} from 'lucide-react';
...
export default function AppSettingsClient({ initialData, globalSettings, formFieldsData }: AppSettingsClientProps) {
    ...
    const handleBreadDragEnd = (result: DropResult) => {
        if (!result.destination) return;
        
        const currentSelectedBreads = formData.meal_page_options.breads || [];
        const activeBreads = orderedBreads.filter((b: string) => currentSelectedBreads.includes(b));
        
        const sourceIndex = result.source.index;
        const destIndex = result.destination.index;
        
        if (sourceIndex >= activeBreads.length || destIndex >= activeBreads.length) {
            // Do not allow dragging unselected items
            return;
        }

        const newSelectedBreads = [...activeBreads];
        const [reorderedItem] = newSelectedBreads.splice(sourceIndex, 1);
        newSelectedBreads.splice(destIndex, 0, reorderedItem);

        setFormData({
            ...formData,
            meal_page_options: {
                ...formData.meal_page_options,
                breads: newSelectedBreads
            }
        });
    };

    const handleCookieDragEnd = (result: DropResult) => {
        if (!result.destination) return;
        
        const currentSelectedCookies = formData.meal_page_options.cookies || [];
        const activeCookies = orderedCookies.filter((c: string) => currentSelectedCookies.includes(c));
        
        const sourceIndex = result.source.index;
        const destIndex = result.destination.index;
        
        if (sourceIndex >= activeCookies.length || destIndex >= activeCookies.length) {
            // Do not allow dragging unselected items
            return;
        }

        const newSelectedCookies = [...activeCookies];
        const [reorderedItem] = newSelectedCookies.splice(sourceIndex, 1);
        newSelectedCookies.splice(destIndex, 0, reorderedItem);

        setFormData({
            ...formData,
            meal_page_options: {
                ...formData.meal_page_options,
                cookies: newSelectedCookies
            }
        });
    };
                    ...
                    <CardHeader className="p-8 border-b border-gray-50">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <CardDescription>Select and drag bread types to set their priority order, or use the switch to toggle them on/off.</CardDescription>
                            </div>
                        </div>
                    </CardHeader>
                    <CardContent className="p-8">
                        {globalSettings?.bread_options && globalSettings.bread_options.length > 0 ? (
                            <DragDropContext onDragEnd={handleBreadDragEnd}>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                    {/* Left Column */}
                                    <Droppable droppableId="breads-left">
                                        {(provided) => (
                                            <div 
                                            ...
                                            >
                                                {orderedBreads.slice(0, Math.ceil(orderedBreads.length / 2)).map((bread: string, index) => {
                                                    ...
                                                    return (
                                                        <Draggable 
                                                            key={bread} 
                                                            draggableId={`bread-${bread}`} 
                                                            index={isSelected ? idxInSelected : 999 + index}
                                                                    ...
                                                                    <div className="flex items-center gap-3">
                                                                        {isSelected && (
                                                                            <div 
                                                                                ...
                                                                                className="cursor-grab active:cursor-grabbing p-1.5 hover:bg-orange-100/50 rounded text-orange-400 hover:text-orange-600"
                                                                            >
                                    ...
                                    {/* Right Column */}
                                    <Droppable droppableId="breads-right">
                                        {(provided) => (
                                            <div 
                                                ref={provided.innerRef} 
                                                {...provided.droppableProps} 
                                                className="flex flex-col gap-4 min-h-[50px]"
                                            >
                                                {orderedBreads.slice(Math.ceil(orderedBreads.length / 2)).map((bread: string, index) => {
                                                    const isSelected = formData.meal_page_options.breads?.includes(bread);
                                                    const currentSelectedBreads = formData.meal_page_options.breads || [];
                                                    const idxInSelected = currentSelectedBreads.indexOf(bread);
                                                    const absoluteIndex = Math.ceil(orderedBreads.length / 2) + index;
                                                    
                                                    return (
                                                        <Draggable 
                                                            key={bread} 
                                                            draggableId={`bread-${bread}`} 
                                                            index={isSelected ? idxInSelected : 999 + absoluteIndex}
                                                            isDragDisabled={!isSelected}
                                                        >
                                                            {(dragProvided, snapshot) => (
                                                                <div 
                    ...
                    <CardHeader className="p-8 border-b border-gray-50">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <CardDescription>Select and drag cookie types to set their priority order, or use the switch to toggle them on/off.</CardDescription>
                            </div>
                        </div>
                    </CardHeader>
                    <CardContent className="p-8">
                        {globalSettings?.cookie_options && globalSettings.cookie_options.length > 0 ? (
                            <DragDropContext onDragEnd={handleCookieDragEnd}>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                    {/* Left Column */}
                                    <Droppable droppableId="cookies-left">
                                        {(provided) => (
                                            <div 
                                                ref={provided.innerRef} 
                                                {...provided.droppableProps} 
                                                className="flex flex-col gap-4 min-h-[50px]"
                                            >
                                                {orderedCookies.slice(0, Math.ceil(orderedCookies.length / 2)).map((cookie: string, index) => {
                                                                ...
                                                                >
                                                                    <div className="flex items-center gap-3">
                                                                        {isSelected && (
                                                                            <div 
                                                                                {...dragProvided.dragHandleProps} 
                                                                                className="cursor-grab active:cursor-grabbing p-1.5 hover:bg-amber-100/50 rounded text-amber-400 hover:text-amber-600"
                                                                            >
                                                                                <GripVertical className="size-4" />
                                                                            </div>
                                                                        )}
                                    ...
                                    {/* Right Column */}
                                    <Droppable droppableId="cookies-right">
                                        {(provided) => (
                                            <div 
                                                ref={provided.innerRef} 
                                                {...provided.droppableProps} 
                                                className="flex flex-col gap-4 min-h-[50px]"
                                            >
                                                {orderedCookies.slice(Math.ceil(orderedCookies.length / 2)).map((cookie: string, index) => {
                                                    const isSelected = formData.meal_page_options.cookies?.includes(cookie);
                                                    const currentSelectedCookies = formData.meal_page_options.cookies || [];
                                                    const idxInSelected = currentSelectedCookies.indexOf(cookie);
                                                    const absoluteIndex = Math.ceil(orderedCookies.length / 2) + index;
